# agy-delegation-criteria Specification

## Purpose

agy（Antigravity CLI）と Claude Code のどちらが実装を担当するかを判定する基準を定義する。タスク単位の難易度判定により、agy をデフォルトとしつつ Claude Code が直接実装すべき条件を明確にする。
